...
public:
    Handler (u_short udp_port, const char *ip_addr, const ACE_TCHAR *a_interface, ACE_Reactor & ); // Constructor.
...
private:
    ACE_SOCK_Dgram_Mcast mcast_; // Multicast wrapper.
    ACE_INET_Addr sockmc_addr_; // Address to multicast to.
...
int Handler::handle_input (ACE_HANDLE h) {
    ...
    if (h == ACE_STDIN) {
        ...
        ssize_t result = this->mcast_.recv (buf, sizeof buf, remote_addr);

        if (result != -1) {
            ACE_DEBUG ((LM_DEBUG,"received datagram from host %s on port %d bytes = %d\n", remote_addr.get_host_name (),remote_addr.get_port_number (),result));
...
int Handler::handle_close (ACE_HANDLE h, ACE_Reactor_Mask) {
...
Handler::Handler (u_short udp_port, const char *ip_addr, const ACE_TCHAR *a_interface, ACE_Reactor &reactor) {
    ...
    else if (reactor.register_handler (this->mcast_.get_handle (), this, ACE_Event_Handler::READ_MASK) == -1)
        ACE_ERROR ((LM_ERROR, "%p\n", "can't register with Reactor\n"));
    // Register the STDIN handler.
    else if (ACE_Event_Handler::register_stdin_handler (this,ACE_Reactor::instance (),ACE_Thread_Manager::instance ()) == -1)
...
int ACE_TMAIN (int argc, ACE_TCHAR *argv[]) {
    ...
    ACE_DEBUG ((LM_DEBUG, "talker Done.\n"));
    return 0;
}